function [centers,clusters,errors,ind] = kmeans_clustersModified(sD, method, n_max, c_max, verbose)
...
if nargin < 2 | isempty(method) | isnan(method), method = 'batch'; end
if nargin < 3 | isempty(n_max) | isnan(n_max), n_max = ceil(sqrt(dlen)); end
...
for i = 2:n_max, % number of clusters
  ...
  for j = 1:c_max     % run number j for cluster i      
    ...
    %MODIFICATION OF THE KMEANS FUNCTION
    %Matlab's kmeans function is used instead of the toolbox's function,
    %new lines are added to compute the output err as the toolbox does
    %Kike 07/10/2019
    [c, k, err] = som_kmeans(method, D, i, 100, 0); %Toolbox kmeans function
%     [k, c, clusErr] = kmeans(D, i); %Matlab kmeans function
%     %New lines to match old outputs
%     k = k';
%     err = sum(clusErr);
