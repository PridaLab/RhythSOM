function RhythSOM_plot(Data, sMap, clusData, clussMap, BMUs, waveletParams, dirSave)
...
% Modified by LCN - E.F Rodríguez Sebastián 2019
    ...
    if nargin < 6 || isempty(waveletParams)
        waveletParams = {};
        fs = 1;
        numEventPlot = 1;
    end
    ...
    % FIGURE 1    
    fig1 = figure('pos',[950 200 700 900]); hold on
    % Cluster colouring
    colors = colormap('hsv');
    ...
    if ismember(0, clussMap)
        for iClus = 1:clusNum
            ...
            for p = 1:length(xs)
                t = text(xs(p), ys(p), num2str(iClus), 'FontSize',14);
    ...
    ax.Position = [left bottom ax_width ax_height-0.07];
    title('Clusters Topology')
    ...
    for ii = 1:size(numEventsMat, 2)
        for jj = 1:size(numEventsMat, 1)
            if numEventsMat(jj,ii) ~= 0
                t = text(ii, jj, num2str(numEventsMat(jj,ii)), 'FontSize',14);
                set(t, 'HorizontalAlignment', 'center');
            end
        end
    end
    ...
    ax_height = outerpos(4) - ti(2) - ti(4);
    ax.Position = [left bottom ax_width ax_height-0.07];
    title('Number of Events per SOM Unit')
    
    
    % FIGURE 3
    fig3 = figure('pos',[150 200 700 900]);
    ...
    for iClus = 1:clusNum
        ...
        % Axes
        %col = unique(Code(find(clussMap == iClus),:),'rows');
        perc = (sum(numEvents(find(clussMap == iClus)))/size(clusData, 1))*100;
        title(sprintf('Cluster #%d (%2.2f %% of events)',iClus, perc)) %, 'Color', col)
    end
    sgtitle('Clusters mean wave')
